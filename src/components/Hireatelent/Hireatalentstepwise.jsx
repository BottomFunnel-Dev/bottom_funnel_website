...
export const Hireatalentstepwise = () => {
    ...
    const [developmentcat , setDevelopmentcat] = useState("")
  ...
  const hiretalent = hireatalentdata.filter((elem)=>{
    if(elem.category === "A"){
        return (
            elem.co
        )
...
<div className='hireatalentmaindatamappingbox'>{hireatalentdata.map((e)=>{
    return (
        <div className='hireatalentcategorieswisesetting'>{
            e.category=="A" ? e.cource:""
        }</div>
    )
})}
